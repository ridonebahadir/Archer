...
public class ArrowController : MonoBehaviour
{
    ...
    public GameObject topla;
    private void Awake()
    {
        //player = GameObject.FindGameObjectWithTag("Player");
        //score = GameObject.FindGameObjectWithTag("Score").transform;
        //screenToWorldPosition = Camera.main.ScreenToWorldPoint(score.transform.position);
    ...
    void Collider(Transform hit)
    {
        if (hit.transform.tag == "Topla")
        {

            //coinManager.AddCoins(hit.transform.position,40,0,Color.green);
            Instantiate(bombParticle, hit.transform.position, Quaternion.identity);
            Instantiate(topla, hit.transform.position + new Vector3(0, 2, 0), Quaternion.identity, transform.parent.parent);
            hit.transform.gameObject.SetActive(false);

            //player.GetComponent<Player>().ToplaHit();
        }
        if (hit.transform.tag == "Player")
        {
            hit.transform.GetComponent<Player>().ArrowHit();
            Instantiate(blood, hit.transform.position, Quaternion.identity);

        }
        if (hit.transform.tag == "Enemy")
        {
            hit.transform.GetComponent<Enemy>().ArrowHit();
            Instantiate(blood, hit.transform.position, Quaternion.identity);
        ...
        {
            Instantiate(healtParticle, hit.transform.position, Quaternion.identity);
            Instantiate(health, hit.transform.position + new Vector3(0, 2, 0), Quaternion.identity, transform.parent.parent);
            hit.transform.gameObject.SetActive(false);
            //player.GetComponent<Player>().HealthHit();
        ...
        if (hit.transform.tag == "Bomb")
        {
            ...
            hit.transform.GetComponent<Bomb>().Explosion();
            hit.transform.gameObject.SetActive(false);
