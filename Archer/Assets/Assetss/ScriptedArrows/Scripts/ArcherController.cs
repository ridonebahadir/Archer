...
public class ArcherController : MonoBehaviour
{
    ...
    public Transform ArrowPosOrigin;
    public bool UseTarget;
    public bool isPlayer;
    
    private void FixedUpdate()
    {
        if (Input.GetKey(KeyCode.E) && (isPlayer))
        {
            // Shoot an Arrow every fixed tick while the key is pressed
            TryToShoot(Target.transform.position);
        }
    }
    ...
    public void TryToShoot(Vector3 target)
    {
    ...
    private void ShootArrow(Vector3 targetPos)
    {
        ...
        if (distance >= MinimalShootRange && distance <= MaximalShootRange)
        {
            ...
            // Create a new Arrow
            var Arrow = Instantiate(ArrowPrefab, ArrowPosOrigin.transform.position, transform.rotation);
